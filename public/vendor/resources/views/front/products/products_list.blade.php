                                        ...
                                        <div class="pull-right">
                                            ...
                                            <?php
												...
												if($total_records > $per_page)
												{
													...
													if ($current_page > 1) {
														$arr_get = Input::get();
														$arr_get['page'] = $current_page - 1;
														$query_string = http_build_query($arr_get);
														
														$pagination_url = url(Request::path().'?'.$query_string);
														echo '<li '.$class.'><a href="'.$pagination_url.'">&lt;</a></li>';
													}
													$x = ($current_page - 3) > 1 ? ($current_page - 3) : 1;
													$y = ($current_page + 3) < $total_page ? ($current_page + 3) : $total_page;
													for($i=$x; $i<= $y; $i++ )
													{
