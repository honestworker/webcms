                          ...
                          <form id="login-form-2" method="post" action="{{ url('/deleteEventItem') }}">
                                <div class="modal-dialog modal-lg">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                        <button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
                                        <h4 class="modal-title" id="myModalLabel2">Delete Product from Special List</h4>
                                        </div><!-- End .modal-header -->
                                        <div class="modal-body clearfix">
				...
				if(response['products'])
				{
					...
					for(var i=0; i < response['products'].length; i++)
					{
						elm = response['products'][i];
                                                products +='<tr style="display:none;" id=product_id_'+ elm.id +'></tr>';
...
{ 
		...
		$.ajax({
			...
			success: function(response){
				...
				$('#successElement').remove();
				 $('#product_id_'+product_id).html("");
				var success = '<td colspan="6"><div id="successElement" class="alert alert-success"><i class="fa fa-check-circle"></i> <strong>Success!</strong><p>Gift has been added successfully.</p></div></td>';
				//$('#form_add_gift').before(success);
                               
				$('#product_id_'+product_id).html(success);
                                $('#product_id_'+product_id).show();
                                
			}
		});	
	
	}
        else if($("#product_color_"+product_id+" input[type=radio]").val() == 0){
          	would_love = (isNaN($('.gift_quantity_'+product_id).val())) ? 1 : $('.gift_quantity_'+product_id).val();
		$.ajax({
			url: '{{ url("/event/addGift") }}',
			type: 'POST',
			dataType: 'json',
			data: '_token=<?php echo csrf_token() ?>&product_id='+product_id+'&event_id=<?php echo Request::segment(2); ?>&would_love='+would_love,			
			success: function(response){
	...
	else
	{
		...
		var error = '<td colspan="6"><div id="errorElement" class="alert alert-danger"><i class="fa fa-times-circle"></i> <strong>Error!</strong><p>Please select product color.</p></div></td>';
	//	$('#form_add_gift').before(error);
                $('#product_id_'+product_id).html("");
         	$('#product_id_'+product_id).html(error);
                $('#product_id_'+product_id).show();
