                    ...
                    <div class="col-md-6 col-sm-12 col-xs-12 product-viewer clearfix">
                        ...
                        <div id="product-image-container">
                            <figure>
                                ...
                                @if($productInfo->list_price != $productInfo->sale_price && $productInfo->list_price!=0)
                    ...
                    <div class="col-md-6 col-sm-12 col-xs-12 product">
                        ...
                        <div class="product-add clearfix">
                        	@if($productInfo->quantity_in_stock)
                                ...
                                <button id="add-to-cart" data-toggle="modal" data-target="#modal-pwp-product-new" class="btn btn-custom-2">ADD TO CART</button>

                                @if (!$pwp_products->isEmpty())
                                <!--START MODAL add pwp product-->
                                    ...
                                    <div class="modal-dialog modal-lg">
                                      <div class="modal-content">
                                        <div class="modal-header">
                                          ...
                                          <h4 id="modal-login-label3" class="modal-title">PWP Product</h4>
                                        </div>
                                        <div class="modal-body">
                                          <div class="form">
                                            ...
                                            <tbody>
                                            @foreach ($pwp_products as $pwp_product)
                                            <tr>
                                                <td><img src="{{ asset('/public/admin/products/medium/'. $pwp_product->product->thumbnail_image_1) }}" alt="<?php echo htmlspecialchars($pwp_product->product->product_name,ENT_QUOTES); ?>" class="img-responsive"></td>
                                                <td>
                                                <a href="product_edit.html">{{$pwp_product->product->product_name}}</a>
                                                <br>
                                                {{$pwp_product->product->product_code}}
                                                </td>
                                                <td>{{number_format($pwp_product->price, 2)}}</td>
                                                <td>{{number_format($pwp_product->product->sale_price, 2)}}</td>
                                                <td>{{number_format($pwp_product->product->list_price, 2)}}</td>
                                                <td>
                                                    <a href="#" class="add-pwp-to-cart btn btn-custom-2">Add To Cart</a>
                                                    <input type="hidden" class="pwp_product_id" value="{{$pwp_product->pwp_product_id}}">
                                                    <input type="hidden" class="pwp_price" value="{{$pwp_product->price}}">
                                                </td>
                                            </tr>
                                            @endforeach
                                            </tbody>
                        ...
                        <div class="product-extra clearfix">
                            ...
                            <ul class="product-list">
                                <li><span>Shipping option(s):</span>
                                    @if (isset($ship_options))
                                    @foreach ($ship_options as $ship)
                                    <a href="#" class="label label-success" data-toggle="modal" data-target="#{{$ship['csv']->id}}">{{$ship['csv']->title}}</a>
                                    @endforeach
                                    @endif
...
47400 Petaling Jaya, Selangor D.E.</li>
                ...
                <div class="row">
                	<div class="col-md-12 col-sm-12 col-xs-12">
                        ...
                        data-flix-brand="<? echo $productInfo->brand_name?>"
                        data-flix-mpn="<?php echo $productInfo->manufacturer_part_number; ?>"
                        data-flix-ean=""
                        data-flix-sku=""
                                        ...
                                        <li>
                                        	...
                                        	@foreach($relatedDiffBrandProducts as $product)
                                                <div class="related-product clearfix">
                                                    ...
                                                    <h5><a href="{{ url('product/' . $product->id) }}"><?php echo (strlen($product->product_name) > 25) ? substr($product->product_name,0,20).'...' : $product->product_name; ?></a></h5>
...
function addToCart(obj)
{
    if (obj) {
        var data = {product_id: $('#product_id').val(),
                    quantity: 1,
                    pwp_product_id: obj.parent().find('.pwp_product_id').val(),
                    pwp_price: obj.parent().find('.pwp_price').val(),
...
$(document).ready(function() {
    $('.add-pwp-to-cart').click(function(){
        if (clickable == false) {
            return false;
        }
        addToCart($(this));
